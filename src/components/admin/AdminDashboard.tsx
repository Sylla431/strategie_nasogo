...
const SECTION_META: Record<AdminSection, { title: string; description: string }> = {
  courses: {
    title: "Cours",
    description: "Créer des cours, gérer les vidéos et consulter le catalogue.",
  },
  access: {
    title: "Accès",
    description: "Accorder l'accès aux cours et suivre les utilisateurs inscrits.",
  },
  telegram: {
    title: "Telegram VIP",
  ...
  return (
    <>
      ...
      <header className="space-y-1">
        <h1 className="text-2xl sm:text-3xl font-semibold text-neutral-900">{meta.title}</h1>
                      ...
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="text-left min-w-0">
                          ...
                          <p className="text-sm text-neutral-600">
                            {course.price.toLocaleString("fr-FR")} FCFA • {allVideos.length} vidéo
                            {allVideos.length > 1 ? "s" : ""}
                          </p>
                        </div>
                      </div>
                      <span className="text-neutral-400 text-lg ml-2">{isExpanded ? "▲" : "▼"}</span>
                    </button>
                    {isExpanded && (
                      <div className="border-t border-neutral-200 p-4 bg-neutral-50/50 space-y-2">
                        {allVideos.length === 0 ? (
